...
export default function StudentRecord() {
  ...
  useEffect(() => {
    const fetchGradeLevels = async () => {
      const token = sessionStorage.getItem("token");
      if (!token) return alert("Missing authorization token for fetching grade levels.");
      try {
        ...
        setModal({ show: true, title: "❌ Error", message: "Failed to fetch grade levels", variant: "danger" });
      }
    };
    fetchGradeLevels();
  }, []);
  ...
  const handleUpload = async (e) => {
    ...
    formData.append("grade_level_id", form.grade_level_id);
    formData.append("grade_level_name", form.grade_level_name);
    ...
    <div className="container py-4">
      ...
      <div className="card shadow-sm border-0">
        <div className="card-header bg-success text-white d-flex justify-content-between align-items-center">
          ...
          <Link to={`/record_student/${lrn}`} className="btn btn-light btn-sm rounded-pill fw-bold shadow-sm">
            View Student Record
          </Link>
        </div>
